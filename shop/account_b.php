...
//делаем запрос в БД
$sql = "SELECT * FROM `users_b` WHERE id=". $user_id_b;
//получаем результат
...
<div class="col-6 ml-5">	
	<form method="POST" id="form_change-user" action="parts/account_b/change_user.php">
	  ...
	  <div class="form-row">
	   ...
	   <div class="form-row">
	    <div class="form-group col-md-12">
	      ...
	      <input type="text" name="info" class="form-control" placeholder="<?php echo $user['info'] ?>">
	    </div>
	  </div>
...
if (isset($_POST["password"])) {
	...
	//делаем запрос в БД
	$sql_password = "SELECT * FROM `users_b` WHERE `users_b`.`id` =". $_COOKIE["user_id_b"];
	//получаем результат
	...
	if ($user['password'] == $password) {
		...
		if ($_POST["new_password"] == $_POST["return_password"]) {
			...
			$sql_change = "UPDATE `users_b` SET `password` = '" . $password_new . "' WHERE `users_b`.`id` =". $_COOKIE["user_id_b"];
			//получаем результат
